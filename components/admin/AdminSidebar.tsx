    ...
    <aside className={`fixed inset-y-0 left-0 z-[70] w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="p-8 pb-4">
        ...
        <nav className="space-y-1">
          <NavItem tab={DashboardTab.OVERVIEW} icon={<LayoutDashboard size={18}/>} label="Overview" />
          <NavItem tab={DashboardTab.PROJECTS} icon={<Briefcase size={18}/>} label="Projects" />
          <NavItem tab={DashboardTab.BLOGS} icon={<FileText size={18}/>} label="Journal" />
          <NavItem tab={DashboardTab.PROFILE} icon={<User size={18}/>} label="Profile" />
        </nav>
      </div>
