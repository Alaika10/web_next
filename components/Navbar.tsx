    ...
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-[150] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          ...
          scrolled || isMenuOpen
          ? 'py-3 bg-white/80 dark:bg-slate-950/90 backdrop-blur-2xl border-b border-slate-200/40 dark:border-indigo-500/10 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.03)] dark:shadow-none' 
          : 'py-6 bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 md:px-12 h-14">
          <Link href="/" className="flex items-center gap-3 group relative z-[160] active:scale-95 transition-transform">
            ...
            <span className="font-black text-lg tracking-tighter uppercase text-slate-800 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
              DataLab
            </span>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center gap-1 bg-slate-100/40 dark:bg-slate-900/50 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200/30 dark:border-white/5">
            {links.map((link) => (
              <Link
                key={link.path}
                href={link.path}
          ...
          <div className="flex items-center gap-3 relative z-[160]">
            ...
            <button 
              ...
              className="p-2.5 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white md:hidden transition-all hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-90 border border-slate-200/20 dark:border-white/5"
      ...
      >
        ...
        <div className={`relative h-full flex flex-col pt-32 pb-12 px-10 overflow-y-auto ${isMenuOpen ? 'animate-premium-in' : ''}`}>
          <div className="flex-1 space-y-2">
            <p className="font-mono text-[9px] uppercase tracking-[0.4em] text-indigo-600 dark:text-indigo-400 font-black mb-8 opacity-70">
              Lab_System_Index
            </p>
              ...
              {links.map((link, idx) => (
                ...
                >
                  ...
                  <div className={`w-12 h-12 rounded-2xl border flex items-center justify-center transition-all duration-500 ${
                    ...
                    : 'border-slate-100 dark:border-slate-800 text-slate-300 dark:text-slate-600 group-hover:border-indigo-600 dark:group-hover:border-indigo-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 group-hover:bg-indigo-50/50 dark:group-hover:bg-indigo-500/5'
                  }`}>
          ...
          <div className={`mt-12 space-y-8 ${isMenuOpen ? 'nav-item-stagger' : 'opacity-0'}`} style={{ animationDelay: '450ms' }}>
            <div className="grid grid-cols-2 gap-4">
              {mounted && (
                isAuth ? (
                  <Link href="/admin" className="p-6 bg-slate-50 dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 active:scale-95 transition-transform group">
                    <LayoutDashboard className="text-indigo-600 dark:text-indigo-400 mb-2 group-hover:scale-110 transition-transform" size={24} />
                    <p className="font-black text-xs uppercase tracking-widest text-slate-800 dark:text-slate-200">Admin Console</p>
                  </Link>
                ) : (
                  <Link href="/login" className="p-6 bg-slate-50 dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 active:scale-95 transition-transform group">
                    <UserIcon className="text-slate-400 dark:text-slate-500 mb-2 group-hover:scale-110 transition-transform" size={24} />
                    <p className="font-black text-xs uppercase tracking-widest text-slate-800 dark:text-slate-200">Researcher Login</p>
                  </Link>
                )
              )}
              <Link href="https://github.com" className="p-6 bg-slate-50 dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 active:scale-95 transition-transform group">
                <Globe className="text-slate-400 dark:text-slate-500 mb-2 group-hover:scale-110 transition-transform" size={24} />
                <p className="font-black text-xs uppercase tracking-widest text-slate-800 dark:text-slate-200">Open Source</p>
              </Link>
            </div>
            
            <div className="flex justify-between items-center px-2 pt-6 border-t border-slate-100 dark:border-slate-800">
