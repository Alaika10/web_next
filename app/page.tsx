            ...
            {projects.map((project, idx) => (
              <Link key={project.id} href={`/projects/${project.id}`} className="group relative bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden border border-slate-200 dark:border-slate-800 hover:shadow-xl dark:hover:shadow-indigo-950/30 transition-all">
                <div className="h-[300px] overflow-hidden relative bg-slate-100 dark:bg-slate-900">
                  ...
                  <div className="absolute inset-x-6 bottom-20 rounded-2xl border border-white/20 bg-slate-950/55 px-4 py-3 text-sm text-slate-100 backdrop-blur-sm opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    <p className="line-clamp-3">{project.description || 'Eksplorasi lengkap ada di detail proyek ini.'}</p>
                  </div>
